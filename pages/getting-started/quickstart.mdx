...
async function main() {
  ...
  console.log('✅ Recipient added to whitelist:', recipient.toBase58());
...
async function main() {
  ...
  try {
    ...
    const vault = await client.getVault(vaultAddress);
    console.log('\n📊 Vault Status After Transaction:');
    ...
    if (error.overrideRequested) {
      ...
      console.log('📱 Override requested. Vault owner will receive notification.');
      console.log('🔗 Blink URL:', error.blinkUrl);
      console.log('\n💡 Vault owner can approve by clicking the Blink URL');
    } else {
...
📱 Override requested. Vault owner will receive notification.
🔗 Blink URL: https://aegis-guardian-production.up.railway.app/api/actions/...
```
...
- ✅ Created an Aegis vault with spending policies
- ✅ Funded the vault and configured whitelist
- ✅ Executed agent transactions within policy
- ✅ Tested the override flow for blocked transactions
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
  ...
  <div className="border rounded-lg p-4">
    ...
    [View SDK Docs →](/sdk/api-reference)
  </div>
